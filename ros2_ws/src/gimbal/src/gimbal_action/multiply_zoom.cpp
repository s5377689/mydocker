...
void MultiplyZoom::run()
{
    ...
    while (!halt_requested_)
    {
        ...
        if (std::abs(zoom_dif) < zoom_tolerance_)
        {
            ...
            std::cout << "[MultiplyZoom] Action completed." << std::endl;
            return;
        }
        else if (zoom_dif > 0)
        {
    ...
    gimbal_controller_.enqueueCommand(cmd);
    std::cout << "[MultiplyZoom] Action halted." << std::endl;
}
