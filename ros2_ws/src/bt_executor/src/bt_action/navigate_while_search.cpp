...
NavigateWhileSearch::NavigateWhileSearch(
    ...
    ))
{
    ...
    gps_sub_ = this->create_subscription<geographic_msgs::msg::GeoPoseStamped>(
        "/ap/geopose/filtered",
        qos,
        std::bind(&NavigateWhileSearch::geopose_callback, this, std::placeholders::_1)
    );
...
NavigateWhileSearch::~NavigateWhileSearch()
{
    ...
    if (navigate_thread_.joinable())
        navigate_thread_.join();
    if (search_process_thread_.joinable())
        search_process_thread_.join();
    if (search_publish_thread_.joinable())
        search_publish_thread_.join();
    RCLCPP_INFO(get_logger(), "All threads have exited.");
}

void NavigateWhileSearch::initialize(
...
void NavigateWhileSearch::run_action_()
{
    navigate_thread_ = std::thread(&NavigateWhileSearch::navigate_loop, this);
    search_process_thread_ = std::thread(&NavigateWhileSearch::search_process_loop, this);
    search_publish_thread_ = std::thread(&NavigateWhileSearch::search_publish_loop, this);

    if (navigate_thread_.joinable()) {
        navigate_thread_.join();
        RCLCPP_INFO(get_logger(), "Navigate thread has exited.");
    }
    if (search_process_thread_.joinable()) {
        search_process_thread_.join();
        RCLCPP_INFO(get_logger(), "Search process thread has exited.");
    }
    if (search_publish_thread_.joinable()) {
        search_publish_thread_.join();
        RCLCPP_INFO(get_logger(), "Search publish thread has exited.");
    }
    ...
    if (target_found_) {
        std::string result_msg {
            "Target found during navigation and search."
        };
        ...
        *result_msg_ = result_msg;
        ...
        std::string result_msg {
            "Reached all waypoints but target not found."
        };
        RCLCPP_INFO(get_logger(), "%s", result_msg.c_str());
        *result_msg_ = result_msg;
        status_ = BT::NodeStatus::FAILURE;
    } else {
        // Action was halted or an error occurred
        status_ = BT::NodeStatus::FAILURE;
    }
...
//                          Navigate
// ============================================================
void NavigateWhileSearch::navigate_loop()
{
    try {
        ...
        while (!halt_requested_ && rclcpp::ok())
        {
            if (target_found_)
            {
                std::string feedback_msg {
                    "Target found, stopping navigation."
                };
                RCLCPP_INFO(get_logger(), "%s", feedback_msg.c_str());

                auto feedback = std::make_shared<ActionT::Feedback>();
                feedback->running_node = "NavigateWhileSearch";
    ...
    publish_zero_velocity();  // Stop the vehicle when action is halted or completed
}

void NavigateWhileSearch::geopose_callback(
...
void NavigateWhileSearch::publish_zero_velocity()
{
...
//                          Search
// ============================================================
void NavigateWhileSearch::search_process_loop()
{
    ...
    try {
        ...
        while (!halt_requested_ && rclcpp::ok() && !reach_goal_)
        {
...
void NavigateWhileSearch::search_publish_loop()
{
    ...
    while (rclcpp::ok() && !halt_requested_ && !reach_goal_)
    {
...
void NavigateWhileSearch::init_blackboard()
{
    std::lock_guard<std::mutex> lock(blackboard_mutex_);
...
void NavigateWhileSearch::transfer_to(
...
// ---------------------------------------------
void NavigateWhileSearch::define_transition_funcs()
{
    // -------------------------
    //     Start -> Scanning
    // -------------------------
...
// ---------------------------------------------
void NavigateWhileSearch::process_scanning(
    cv::Mat & img)
{    int img_match_idx = -1;
...
void NavigateWhileSearch::process_detecting(
    cv::Mat & img)
{
...
void NavigateWhileSearch::process_recognizing(
    cv::Mat & img)
{
    auto now = std::chrono::steady_clock::now();
...
void NavigateWhileSearch::process_tracking(
    cv::Mat & img)
{
    cv::Rect lockon_bbox;
    bool tracking_success = tracker_->update(img, lockon_bbox);
