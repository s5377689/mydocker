...
void Hold::run_action_()
{
    ...
    while (rclcpp::ok() && !client->service_is_ready()) {
        ...
        if (std::chrono::steady_clock::now() - start_time > std::chrono::seconds(timeout_)) {
            std::string result_msg {
                "Mode switch service not available after waiting for " + std::to_string(timeout_) + " seconds."
            };
    ...
    request->mode = static_cast<u_int8_t>(ArduPilotMode::HOLD);
