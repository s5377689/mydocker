...
public:
    ...
    // ---------------------------------
    int upper_horizon_margin() const { return upper_horizon_margin_; }
    void set_upper_horizon_margin(int margin) { upper_horizon_margin_ = margin; }
    int lower_horizon_margin() const { return lower_horizon_margin_; }
    void set_lower_horizon_margin(int margin) { lower_horizon_margin_ = margin; }
    ...
    int vote_threshold() const { return vote_threshold_; }
    void set_vote_threshold(int threshold) { vote_threshold_ = threshold; }
...
private:
    ...
    int scan_time_ms_ {3000};  // Time to stay in the Scanning state before start blob detection
    ...
    int vote_threshold_ {8}; // Threshold for considering a blob as a possible target
    ...
    // --------------------------------
    std::weak_ptr<gimbal::GimbalStreamer> gimbal_streamer_;
    std::weak_ptr<gimbal::GimbalController> gimbal_controller_;

    // --------------------------------
    //        Target Registry
    // --------------------------------
    std::weak_ptr<system_component::TargetRegistry> target_registry_;

    // --------------------------------
    //      Searching Algorithms
    // --------------------------------
    // Parameters for sea horizon detection
    int upper_horizon_margin_ {20}; // Upper margin for the detected sea horizon
    int lower_horizon_margin_ {20}; // Lower margin for the detected sea horizon

    // MSER blob detector & position voting
    cv::Ptr<cv::MSER> mser_;
    int num_x_bins_ {15}; // Number of bins for horizontal position voting
    std::vector<int> blob_hist_; // Histogram for voting results
    ...
    cv::Rect current_bbox_;  // Current bounding box of the target being tracked

    // --------------------------------
    //      Search State Publisher 
    // --------------------------------
    rclcpp::Publisher<sensor_msgs::msg::CompressedImage>::SharedPtr search_pub_;
};
